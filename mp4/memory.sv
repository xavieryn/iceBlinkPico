...
//
// Data memory spans addresses 0x00000000 to 0x00000FFF and can be written to // 12 bits total??? 2^12 addresses for whatever 
// or read from in words (4 bytes), half words (two bytes), or signle bytes. 
// Word access are aligned to four-byte boundaries. (LIKE IN SOFTSYS) Half-word accesses are 
...
//
// Instruction memory spans addresses 0x00001000 to 0x00001FFF and can only be (RIGHT AFTER Data memory)
...
module memory #(
    ...
    parameter CLK_FREQ = 12000000 // 12 million
)(
    input logic     clk, 
    ...
    output logic    reset, 
    output logic    led,                    // Active-high PWM output for user LED (if active, will turn on)
...
module memory_array #(
    ...
    input logic     [7:0] data_in, // 8 bits (1 byte)
    output logic    [7:0] data_out  // 8 bits (1 byte)
);

    logic [7:0] memory [0:1023]; // what is this 1024
